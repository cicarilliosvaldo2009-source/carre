...
const TIPO_EXAMEN_COLOR = Object.fromEntries(
  TIPOS_EXAMEN.map((t) => [t.id, t.color])
);
...
const colorParaAnio = (anio) =>
  anio ? ANIO_SHADES[Math.min(anio - 1, ANIO_SHADES.length - 1)] : "#DAD4BC";
...
const LIMITE_ARCHIVO_BYTES = 1.5 * 1024 * 1024; // 1.5 MB — todo se guarda dentro del mismo storage
...
function leerArchivoComoDataUrl(file) {
  return new Promise((resolve, reject) => {
    if (file.size > LIMITE_ARCHIVO_BYTES) {
      reject(
        new Error(
          "El archivo pesa más de 1.5 MB. Subilo a Drive u otro servicio y pegá el link en vez de adjuntarlo."
        )
      );
      return;
    }
    ...
    reader.onerror = () => reject(new Error("No se pudo leer el archivo"));
    reader.onload = () =>
      resolve({ dataUrl: reader.result, nombre: file.name, tipo: file.type });
    reader.readAsDataURL(file);
  });
}
...
function seedData() {
  const hoy = new Date();
  const plus = (d) =>
    toDateStr(new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate() + d));
  ...
  const definiciones = [
    // ---- 1er año (todas aprobadas) ----
    {
      ...
      profesor: "Cra. Herrera",
      aula: "Aula 101",
      anio: 1,
      color: "#B5432E",
      estado: "Aprobada",
      fechaAprobada: plus(-650),
      correlativas: "",
      examenes: [
        { tipo: "Parcial", titulo: "Primer parcial", nota: 8 },
        { tipo: "Final", titulo: "Final", nota: 7 },
      ],
      recursos: [
        {
          tipo: "Libro",
          nombre: "Contabilidad Básica — Fowler Newton",
          url: "",
        },
      ],
    },
    ...
    },
    {
      ...
      color: "#3D6B4F",
      estado: "Aprobada",
      fechaAprobada: plus(-610),
      correlativas: "",
      examenes: [{ tipo: "Final", titulo: "Final", nota: 9 }],
    },
    {
      nombre: "Elementos de Derecho Civil",
      profesor: "Dr. Ibáñez",
      aula: "Aula 110",
      anio: 1,
      color: "#6E4C8A",
      estado: "Aprobada",
      fechaAprobada: plus(-600),
      correlativas: "",
      examenes: [{ tipo: "Final", titulo: "Final", nota: 6 }],
    },
    ...
    // ---- 2do año (todas aprobadas, salvo Estadística I que está regular) ----
    {
      ...
      color: "#B5432E",
      estado: "Aprobada",
      fechaAprobada: plus(-320),
      correlativas: "Contabilidad I",
      examenes: [
        { tipo: "Parcial", titulo: "Primer parcial", nota: 7 },
        { tipo: "Final", titulo: "Final", nota: 8 },
      ],
    },
    {
    ...
    },
    {
      ...
      asistencia: { presentes: 14, faltas: 0 },
      examenes: [
        { tipo: "Parcial", titulo: "Primer parcial", nota: 6 },
        { tipo: "Parcial", titulo: "Segundo parcial", nota: 7 },
      ],
    ...
    },
    {
      ...
      color: "#6E4C8A",
      estado: "Aprobada",
      fechaAprobada: plus(-280),
      correlativas: "Elementos de Derecho Civil",
      examenes: [{ tipo: "Final", titulo: "Final", nota: 7 }],
    },
    {
      ...
      color: "#C4842E",
      estado: "Aprobada",
      fechaAprobada: plus(-270),
      correlativas: "Introducción a la Economía",
      examenes: [{ tipo: "Final", titulo: "Final", nota: 8 }],
    },
    ...
    // ---- 3er año (año actual: cursando / regular) ----
    {
      ...
      examenes: [
        { tipo: "Trabajo práctico", titulo: "TP1 — Costeo ABC", nota: 8 },
      ],
          ...
          bloques: [
            {
              ...
              titulo: "Idea central",
              texto:
                "El costeo basado en actividades asigna los costos indirectos según las actividades que efectivamente consumen los productos, en vez de prorratearlos con una única base.",
            },
          ],
        },
      ],
    },
    {
      ...
      tareas: [
        { titulo: "Preparar exposición de EECC consolidados", fecha: plus(5) },
      ],
    },
    {
      nombre: "Derecho Tributario I",
      profesor: "Dr. Ibáñez",
      aula: "Aula 110",
      anio: 3,
      color: "#6E4C8A",
      estado: "Regular",
      correlativas: "Derecho Comercial",
      horarios: [{ dia: "Jueves", inicio: "18:00", fin: "21:00" }],
      asistencia: { presentes: 14, faltas: 0 },
      examenes: [
        { tipo: "Parcial", titulo: "Primer parcial", nota: 7 },
        { tipo: "Parcial", titulo: "Segundo parcial", nota: 6 },
      ],
    },
    {
      ...
      color: "#2C5C8A",
      estado: "Cursando",
      correlativas: "Matemática Financiera, Estadística I",
      horarios: [{ dia: "Viernes", inicio: "10:00", fin: "13:00" }],
      asistencia: { presentes: 9, faltas: 7 },
      examenes: [
        { tipo: "Trabajo práctico", titulo: "TP1 — VAN y TIR", nota: null },
      ],
    ...
    // ---- 4to año (pendiente, todavía no empezado) ----
    {
      ...
      color: "#B5432E",
      estado: "Pendiente",
      correlativas: "Contabilidad Superior, Contabilidad de Costos",
      recursos: [
        { tipo: "Link", nombre: "Normas de Auditoría (FACPCE)", url: "" },
      ],
    },
    {
      ...
      color: "#6E4C8A",
      estado: "Pendiente",
      correlativas: "Derecho Tributario I",
    },
    {
      nombre: "Sistemas de Costos para la Gestión",
      profesor: "Cra. Molina",
      aula: "Aula 201",
      anio: 4,
      color: "#B5432E",
      estado: "Pendiente",
      correlativas: "Contabilidad de Costos",
    },
    {
    ...
    {
      nombre: "Seminario de Práctica Profesional",
      profesor: "Cra. Herrera",
      aula: "Aula 101",
      anio: 4,
      color: "#A65A3C",
      estado: "Pendiente",
      correlativas: "Auditoría",
    },
  ];
...
function useStore() {
  ...
  useEffect(() => {
    if (!cargado) return;
    const t = setTimeout(() => {
      guardarValor(STORAGE_KEY, { materias });
    }, 400);
    return () => clearTimeout(t);
  }, [materias, cargado]);

  return { materias, setMaterias, cargado };
}
      ...
      {COLORES.map((c) => (
        <button
          ...
          className={`color-swatch ${
            value === c.hex ? "color-swatch-active" : ""
          }`}
        ...
        {items.map((it) => (
          <button
            ...
            className={`sidebar-item ${
              view === it.id ? "sidebar-item-active" : ""
            }`}
...
function Inicio({ materias, setView, abrirMateria, onCompletarTarea }) {
  ...
  const promedioGeneral = useMemo(() => {
    const notas = materias
      .flatMap((m) => (m.examenes || []).map((e) => e.nota))
  ...
  const pendientesOrdenadas = useMemo(() => {
    const todas = materias.flatMap((m) =>
      (m.tareas || [])
  ...
  const historialAprobadas = useMemo(() => {
    return materias
      .filter((m) => m.estado === "Aprobada" && m.fechaAprobada)
      .sort((a, b) => (a.fechaAprobada < b.fechaAprobada ? 1 : -1))
      .slice(0, 6);
  }, [materias]);
    ...
    <div className="view">
      ...
      <div className="dos-columnas">
        <section className="panel">
          ...
          {!claseHoy && (
            <p className="muted">Es fin de semana, no tenés cursada.</p>
          )}
          {claseHoy && clasesDeHoy.length === 0 && (
            <p className="muted">No tenés clases cargadas para hoy.</p>
          )}
          <ul className="lista-eventos">
            {clasesDeHoy.map((c, i) => (
              <li
                key={i}
                className="evento-fila"
                onClick={() => abrirMateria(c.materia.id)}
              >
                <span
                  className="evento-tipo-dot"
          ...
          <button className="link-btn" onClick={() => setView("calendario")}>
            Ver tu Google Calendar →
          </button>
        </section>
        ...
        <section className="panel">
          ...
          <ul className="lista-materias-mini">
              ...
              .map((m) => (
                <li key={m.id} onClick={() => abrirMateria(m.id)}>
                  ...
                  <div className="lista-materias-texto">
                    ...
                    <span className="muted">
                      {m.horarios.map((h) => h.dia.slice(0, 2)).join(" · ") ||
                        "Sin horario"}
                    </span>
                  </div>
                </li>
            ...
            {materias.filter((m) => m.estado === "Cursando").length === 0 && (
              <p className="muted">
                No estás cursando ninguna materia por ahora.
              </p>
            )}
          </ul>
          <button className="link-btn" onClick={() => setView("materias")}>
      ...
      {pendientesOrdenadas.length > 0 && (
        <section className="panel panel-pendientes-grande">
          <div className="panel-pendientes-head">
            <h2>Pendientes</h2>
            <span className="panel-pendientes-contador">
          ...
          <ul className="lista-pendientes-grande">
            {pendientesOrdenadas.map((t) => (
              <li
                key={t.id}
                className={`pendiente-fila-grande pendiente-${nivelUrgencia(
                  t.fecha
                )}`}
              >
                <button
                  className="tarea-check"
                  onClick={() => onCompletarTarea(t.materia.id, t.id)}
                  title="Marcar como hecha"
                >
                  <Square size={19} />
                </button>
                <div
                  className="pendiente-texto-grande"
                  onClick={() => abrirMateria(t.materia.id)}
                >
                  <strong>{t.titulo}</strong>
                  <span className="muted">{t.materia.nombre}</span>
                </div>
                <span
                  className={`tarea-fecha tarea-fecha-${nivelUrgencia(
                    t.fecha
                  )}`}
                >
                  {textoUrgencia(t.fecha)}
                </span>
              </li>
            ))}
          </ul>
      ...
      {historialAprobadas.length > 0 && (
        <section className="panel historial-panel">
          <h2>Historial de aprobadas</h2>
          <ul className="lista-historial">
            {historialAprobadas.map((m) => (
              <li key={m.id} onClick={() => abrirMateria(m.id)}>
                <span className="tab-color" style={{ background: m.color }} />
                <strong>{m.nombre}</strong>
                <span className="muted">{fmtFechaLarga(m.fechaAprobada)}</span>
              </li>
            ))}
          </ul>
        </section>
...
function FilaMateria({
  ...
  return (
    <div
      className={`fila-materia ${inactiva ? "fila-materia-inactiva" : ""}`}
      onClick={() => onOpen(m.id)}
    >
      ...
      <div className="fila-texto">
        <div className="fila-nombre">
          ...
          {pendientes.length > 0 && (
            <span
              className="fila-lock"
              title={`Requiere: ${pendientes.join(", ")}`}
            >
              <Lock size={12} />
            </span>
          )}
      ...
      {promedio !== null && (
        <span className="fila-promedio">{promedio.toFixed(1)}</span>
      )}
      <span
        className="fila-pill"
        style={{ "--sc": ESTADO_COLOR[m.estado] }}
        title={
          m.estado === "Aprobada" && m.fechaAprobada
            ? `Aprobada el ${fmtFechaLarga(m.fechaAprobada)}`
            : undefined
        }
      >
        {m.estado}
            ...
            <div className="fila-menu-pop">
              ...
              {ESTADOS.filter((e) => e !== m.estado).map((e) => (
                <button
                  ...
                  onClick={() => {
                    setMenuAbierto(false);
                    onCambiarEstado(m.id, e);
                  }}
                >
                  <span
                    className="fila-menu-dot"
...
function MateriaFormModal({ materia, onSave, onClose }) {
  ...
  const delHorario = (i) =>
    set(
      "horarios",
      form.horarios.filter((_, idx) => idx !== i)
    );
    ...
    >
      <div className="form-grid">
        ...
        <label className="campo campo-full">
          ...
          <input
            value={form.correlativas}
            onChange={(e) => set("correlativas", e.target.value)}
            placeholder="Ej: Programación I, Álgebra"
          />
        </label>
        ...
        <div className="campo campo-full">
          ...
          {form.horarios.map((h, i) => (
            <div key={i} className="horario-fila">
              <select
                value={h.dia}
                onChange={(e) => updHorario(i, "dia", e.target.value)}
              >
                {DIAS.map((d) => (
              ...
              <input
                type="time"
                value={h.inicio}
                onChange={(e) => updHorario(i, "inicio", e.target.value)}
              />
              <span className="muted">a</span>
              <input
                type="time"
                value={h.fin}
                onChange={(e) => updHorario(i, "fin", e.target.value)}
              />
              <IconBtn
                icon={Trash2}
                title="Quitar"
                danger
                onClick={() => delHorario(i)}
              />
            </div>
          ))}
          <button
        ...
        <label className="campo campo-full">
          ...
          <textarea
            ...
            placeholder="Notas generales sobre la cursada"
          />
        </label>
      </div>
...
function MateriaDetalle({
  ...
  const [resumenActivo, setResumenActivo] = useState(
    materia.resumenes[0]?.id || null
  );
  ...
  const updAsistencia = (fields) =>
    patch({ asistencia: { ...asistencia, ...fields } });
  const sumarPresente = () =>
    updAsistencia({ presentes: asistencia.presentes + 1 });
  const sumarFalta = () => updAsistencia({ faltas: asistencia.faltas + 1 });
  const restarPresente = () =>
    updAsistencia({ presentes: Math.max(0, asistencia.presentes - 1) });
  const restarFalta = () =>
    updAsistencia({ faltas: Math.max(0, asistencia.faltas - 1) });
    ...
    patch({
      examenes: [
        ...examenes,
        {
          ...
          nota: nuevoExamen.nota === "" ? null : Number(nuevoExamen.nota),
        },
      ],
    });
  ...
  const updNotaExamen = (id, valor) => {
    patch({
      examenes: examenes.map((e) =>
        e.id === id ? { ...e, nota: valor === "" ? null : Number(valor) } : e
      ),
    });
  };
  ...
  const delRecurso = (id) =>
    patch({ recursos: materia.recursos.filter((r) => r.id !== id) });
      ...
      resumenes: materia.resumenes.map((r) =>
        r.id === resumenId
          ? {
              ...
              bloques: [
                ...r.bloques,
                { id: uid(), tipo, titulo: "", texto: "", imagen: "" },
              ],
            }
          : r
      ),
    ...
    patch({
      resumenes: materia.resumenes.map((r) =>
        r.id === resumenId
          ? {
              ...r,
              bloques: r.bloques.map((b) => (b.id === bloque.id ? bloque : b)),
            }
          : r
      ),
    });
  ...
  const delBloque = (resumenId, bloqueId) => {
    patch({
      resumenes: materia.resumenes.map((r) =>
        r.id === resumenId
          ? { ...r, bloques: r.bloques.filter((b) => b.id !== bloqueId) }
          : r
      ),
    });
  };
    ...
    >
      <div className="detalle-panel">
        <div className="detalle-head" style={{ "--mc": materia.color }}>
          <button className="volver" onClick={onClose}>
        ...
        <div className="detalle-titulo">
          <div className="detalle-titulo-fila">
            ...
            {promedio !== null && (
              <span className="detalle-promedio">
          ...
          <p className="muted">
            {materia.profesor || "Sin docente"}{" "}
            {materia.aula ? `· ${materia.aula}` : ""} ·{" "}
            {anioLabel(materia.anio)}
          </p>
          {materia.estado === "Aprobada" && materia.fechaAprobada && (
            <p className="detalle-fecha-aprobada">
              <GraduationCap size={13} /> Aprobada el{" "}
              {fmtFechaLarga(materia.fechaAprobada)}
            </p>
          )}
            ...
            <div className="detalle-horarios">
              {materia.horarios.map((h, i) => (
                <span key={i} className="chip">
                  <Clock size={12} /> {h.dia} {h.inicio}–{h.fin}
                </span>
              ))}
            </div>
          )}
        ...
        <div className="tabs">
          {[
            ...
            const hayPendientes =
              t === "tareas" && tareas.some((x) => !x.completada);
            return (
              <button
                key={t}
                className={`tab ${tab === t ? "tab-activo" : ""} ${
                  hayPendientes ? "tab-con-alerta" : ""
                }`}
                onClick={() => setTab(t)}
              >
                {t === "info"
            ...
            <ul className="lista-tareas">
              {tareas
                ...
                .map((t) => (
                  <li
                    key={t.id}
                    className={t.completada ? "tarea-completada" : ""}
                  >
                    <button
                      className="tarea-check"
              ...
              {tareas.length === 0 && (
                <p className="muted">
                  No tenés tareas pendientes en esta materia.
                </p>
              )}
            </ul>
            <button
              className="btn-agregar-ancho"
              onClick={() => setTareaModalAbierto(true)}
            >
        ...
        {tareaModalAbierto && (
          <Modal
            title="Nueva tarea"
            onClose={() => setTareaModalAbierto(false)}
          >
            <div className="form-grid">
              <label className="campo campo-full">
                <span>Nombre</span>
                <input
                  ...
                  onChange={(e) =>
                    setNuevaTarea((t) => ({ ...t, titulo: e.target.value }))
                  }
              ...
              <label className="campo campo-full">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNuevaTarea((t) => ({ ...t, fecha: e.target.value }))
                  }
                />
              </label>
            </div>
            <div className="modal-acciones">
              <button
                ...
                onClick={() => setTareaModalAbierto(false)}
              >
                Cancelar
              </button>
              <button className="btn-primario" onClick={addTarea}>
                Agregar
              </button>
            </div>
          </Modal>
                    ...
                    <circle
                      ...
                      stroke={
                        asistenciaPct >= ASISTENCIA_MINIMA
                          ? "#6FB37E"
                          : asistenciaPct >= ASISTENCIA_MINIMA - 15
                          ? "var(--ochre)"
                          : "var(--brick)"
                      }
                      strokeWidth="7"
              ...
              <div className="asistencia-datos">
                <p
                  className={`asistencia-estado ${
                    asistenciaPct !== null && asistenciaPct < ASISTENCIA_MINIMA
                      ? "asistencia-estado-riesgo"
                      : ""
                  }`}
                >
                  {asistenciaPct === null
                ...
                <span className="muted">
                  {asistencia.presentes} presentes · {asistencia.faltas} faltas
                </span>
              </div>
            </div>
            ...
            <div className="asistencia-controles">
              <div className="asistencia-control">
                ...
                <div className="asistencia-stepper">
                  <IconBtn
                    icon={Minus}
                    title="Restar presente"
                    onClick={restarPresente}
                  />
                  <strong>{asistencia.presentes}</strong>
                  <IconBtn
                    icon={Plus}
                    title="Sumar presente"
                    onClick={sumarPresente}
                  />
                </div>
              </div>
              <div className="asistencia-control">
          ...
          <div className="tab-panel">
            ...
            <ul className="lista-examenes">
              {examenes.map((e) => (
                <li key={e.id}>
                  <span
                    className="chip-tipo"
                    style={{ "--tc": TIPO_EXAMEN_COLOR[e.tipo] }}
                  >
                    {e.tipo}
                  </span>
                  ...
                  <IconBtn
                    ...
                    title="Eliminar"
                    danger
                    onClick={() => delExamen(e.id)}
                  />
                </li>
              ))}
              {examenes.length === 0 && (
                <p className="muted">
                  Todavía no cargaste trabajos ni exámenes.
                </p>
              )}
            </ul>
            <div className="nuevo-examen">
              <select
                ...
                onChange={(e) =>
                  setNuevoExamen((n) => ({ ...n, tipo: e.target.value }))
                }
              >
                {TIPOS_EXAMEN.map((t) => (
              ...
              <input
                ...
                onChange={(e) =>
                  setNuevoExamen((n) => ({ ...n, titulo: e.target.value }))
                }
              />
              <input
                type="number"
                ...
                onChange={(e) =>
                  setNuevoExamen((n) => ({ ...n, nota: e.target.value }))
                }
              />
              <button className="btn-secundario btn-chico" onClick={addExamen}>
        ...
        {tab === "recursos" && (
          <div className="tab-panel">
            <ul className="lista-recursos">
                ...
                return (
                  <li key={r.id}>
                    ...
                    {r.archivo && (
                      <a
                        href={r.archivo}
                        download={r.archivoNombre || r.nombre}
                        className="icon-btn"
              ...
              {materia.recursos.length === 0 && (
                <p className="muted">Todavía no agregaste recursos.</p>
              )}
            </ul>
            <div className="nuevo-recurso">
              <select
                value={nuevoRecurso.tipo}
                onChange={(e) =>
                  setNuevoRecurso((r) => ({ ...r, tipo: e.target.value }))
                }
              >
                {RECURSO_TIPOS.map((t) => (
              ...
              <input
                ...
                onChange={(e) =>
                  setNuevoRecurso((r) => ({ ...r, nombre: e.target.value }))
                }
              />
              <input
                ...
                onChange={(e) =>
                  setNuevoRecurso((r) => ({ ...r, url: e.target.value }))
                }
              />
              <button className="btn-secundario btn-chico" onClick={addRecurso}>
                <Plus size={14} /> Agregar
              </button>
            </div>
            <div className="adjuntar-archivo">
              <label className="btn-secundario btn-chico adjuntar-archivo-btn">
                ...
                <input
                  type="file"
                  hidden
                  onChange={(e) => subirArchivoRecurso(e.target.files?.[0])}
                />
              </label>
              ...
              <span className="adjuntar-archivo-ayuda">
                Máx. 1.5 MB — para archivos más grandes, usá el campo URL con un
                link de Drive.
              </span>
            </div>
            {errorArchivo && (
              <p className="adjuntar-archivo-error">{errorArchivo}</p>
            )}
          </div>
        )}
              ...
              {materia.resumenes.map((r) => (
                <div
                  ...
                  className={`resumen-item ${
                    resumenActivo === r.id ? "resumen-item-activo" : ""
                  }`}
                >
                  <button
                    className="resumen-item-btn"
                    onClick={() => setResumenActivo(r.id)}
                  >
                    {r.titulo || "Sin título"}
                  </button>
                  <IconBtn
                    ...
                    title="Eliminar"
                    danger
                    onClick={() => delResumen(r.id)}
                  />
                </div>
              ))}
              <button className="btn-secundario btn-chico" onClick={addResumen}>
                <Plus size={14} /> Nuevo resumen
              </button>
            </div>
              ...
              {resumen ? (
                <>
                  <input
                    ...
                    onChange={(e) =>
                      updResumenTitulo(resumen.id, e.target.value)
                    }
                  />
                  ...
                  <div className="bloque-agregar">
                    <button
                      className="btn-secundario btn-chico"
                      onClick={() => addBloque(resumen.id, "texto")}
                    >
                      <Type size={14} /> Bloque de texto
                    </button>
                    <button
                      className="btn-secundario btn-chico"
                      onClick={() => addBloque(resumen.id, "imagen")}
                    >
                      <ImageIcon size={14} /> Bloque de imagen
                    </button>
                  </div>
                </>
              ) : (
...
function MapaCorrelativas({ materias, abrirMateria }) {
  ...
  const anchoTotal =
    MAPA_PAD * 2 +
    columnas.length * MAPA_NODO_W +
    Math.max(0, columnas.length - 1) * MAPA_COL_GAP;
  const altoTotal =
    MAPA_PAD * 2 + MAPA_HEADER_H + maxFilas * (MAPA_NODO_H + MAPA_ROW_GAP);
  ...
  return (
    <div className="mapa-wrap">
      <div className="mapa-leyenda">
        <span>
          <i className="mapa-leyenda-linea mapa-leyenda-linea-ok" /> Correlativa
          aprobada — vía libre
        </span>
        <span>
          <i className="mapa-leyenda-linea mapa-leyenda-linea-no" /> Correlativa
          pendiente — bloquea
        </span>
      </div>
      <div className="mapa-scroll">
        <div
          ...
          style={{ width: anchoTotal, height: altoTotal }}
        >
          {columnas.map(([anio], colIdx) => (
            <div
              ...
              style={{
                left: MAPA_PAD + colIdx * (MAPA_NODO_W + MAPA_COL_GAP),
                width: MAPA_NODO_W,
              }}
            >
...
function MateriasView({
  ...
  const guardarMateria = (m) => {
    setMaterias((prev) => {
      ...
      const pasaAAprobada =
        m.estado === "Aprobada" &&
        (!existente || existente.estado !== "Aprobada");
      const actualizada = pasaAAprobada
        ? { ...m, fechaAprobada: toDateStr(new Date()) }
        : m;
      return existente
        ? prev.map((x) => (x.id === m.id ? actualizada : x))
        : [...prev, actualizada];
    });
  ...
  const cambiarEstado = (id, nuevoEstado) => {
    setMaterias((prev) =>
      prev.map((m) => {
        if (m.id !== id) return m;
        const pasaAAprobada =
          nuevoEstado === "Aprobada" && m.estado !== "Aprobada";
        return {
          ...m,
          estado: nuevoEstado,
          fechaAprobada: pasaAAprobada
            ? toDateStr(new Date())
            : m.fechaAprobada,
        };
      })
  ...
  const filtradasEstado =
    filtro === "Todas" ? materias : materias.filter((m) => m.estado === filtro);
  const termino = busqueda.trim().toLowerCase();
  const filtradas = termino
    ? filtradasEstado.filter(
        (m) =>
  ...
  useEffect(() => {
    if (vistaAnio !== "Todos" && !aniosPresentes.includes(vistaAnio))
      setVistaAnio("Todos");
  ...
  const listaPlana =
    vistaAnio === "Todos"
      ? null
      : filtradas.filter((m) => (m.anio || 0) === vistaAnio);
    ...
    <div className="view">
      ...
      <div className="materias-toolbar">
        ...
        <div className="modo-toggle">
          <button
            className={modo === "lista" ? "modo-toggle-activo" : ""}
            onClick={() => setModo("lista")}
          >
            <List size={14} /> Lista
          </button>
          <button
            className={modo === "mapa" ? "modo-toggle-activo" : ""}
      ...
      {modo === "mapa" ? (
        <MapaCorrelativas
          materias={materias}
          abrirMateria={setMateriaAbiertaId}
        />
      ) : (
        <>
          ...
          {aniosPresentes.length > 1 && (
            <div className="tabs-anio">
              <button
                className={`tab-anio ${
                  vistaAnio === "Todos" ? "tab-anio-activo" : ""
                }`}
              ...
              {aniosPresentes.map((a) => (
                <button
                  key={a}
                  className={`tab-anio ${
                    vistaAnio === a ? "tab-anio-activo" : ""
                  }`}
                  style={{
                    "--tc": colorParaAnio(a),
                    "--tct": a ? "#F6F3E7" : "var(--ink)",
                  }}
          ...
          <div className="lista-materias-panel">
                  ...
                  ([anio, items]) =>
                    items.length > 0 && (
                      <div key={anio} className="grupo-anio">
                        {aniosPresentes.length > 1 && (
                          <p className="grupo-anio-titulo">
                            {anio === 0 ? "Sin año" : anioLabel(anio)}
                          </p>
                        )}
                        {items.map((m) => (
                          <FilaMateria key={m.id} m={m} {...filaProps} />
                        ))}
                      </div>
                    )
                )
            ...
            {filtradas.length === 0 && (
              <p className="muted lista-materias-vacia">
      ...
      {materiaAbierta && !formAbierto && (
        <MateriaDetalle
          ...
          onDelete={() => eliminarMateria(materiaAbierta.id)}
          onClose={() => setMateriaAbiertaId(null)}
          onEdit={() => {
            setEditando(materiaAbierta);
            setFormAbierto(true);
          }}
        />
      )}
    ...
    <div className="view">
      ...
      <div className="horario-grid-wrap">
        <div
          ...
          style={{
            gridTemplateRows: `40px repeat(${horas.length}, ${rowH}px)`,
          }}
        >
          <div className="horario-esquina" />
          {DIAS.map((d) => (
            <div key={d} className="horario-dia-header">
              {d}
            </div>
          ))}
          ...
          {horas.map((h) => (
            <React.Fragment key={h}>
              <div
                className="horario-hora-label"
                style={{ gridRow: h - HORA_INICIO + 2 }}
              >
              ...
              {bloques
                ...
                .map((b) => {
                  ...
                  const height = Math.max(
                    (parseHora(b.fin) - parseHora(b.inicio)) * rowH - 4,
                    22
                  );
                  return (
                    <button
  ...
  return (
    <Modal title="Conectar tu Google Calendar" onClose={onClose} wide>
      ...
      <div className="ayuda-calendario ayuda-calendario-aviso">
        <p className="ayuda-calendario-titulo">
          ⚠️ Usá un calendario aparte, no tu cuenta personal
        </p>
        <p>
          Si pegás el ID de tu Gmail principal, para que se pueda mostrar acá
          tendrías que hacer
          <strong> público todo tu calendario personal</strong> (cualquiera con
          el link vería todos tus eventos). Mejor creá un calendario nuevo solo
          para la facultad y hacé público ese.
        </p>
      </div>

      <div className="ayuda-calendario">
        <p className="ayuda-calendario-titulo">
          ¿Cómo lo creo y consigo el ID?
        </p>
        <ol>
          <li>
            En Google Calendar, al lado de <strong>"Otros calendarios"</strong>,
            tocá el <strong>+</strong> →{" "}
            <strong>"Crear un calendario nuevo"</strong> (ej: "Facultad").
          </li>
          <li>
            Cargá ahí tus parciales y entregas, en vez de en tu calendario
            principal.
          </li>
          <li>
            En los <strong>tres puntos</strong> junto al nombre del calendario
            nuevo → <strong>Configuración</strong>.
          </li>
          <li>
            En <strong>"Permisos de acceso"</strong> activá{" "}
            <strong>"Hacer disponible al público"</strong>.
          </li>
          <li>
            Bajá hasta <strong>"Integrar calendario"</strong> y copiá el{" "}
            <strong>ID de calendario</strong> (termina en
      ...
      <div className="modal-acciones">
        ...
        <button className="btn-primario" onClick={() => onSave(valor)}>
          Guardar
        </button>
      </div>
    </Modal>
    ...
    <div className="view view-sin-padding-abajo">
      <header className="view-head view-head-calendario">
        ...
        <div className="calendario-acciones-head">
          ...
          >
            <Pencil size={14} />{" "}
            {calendarId ? "Cambiar calendario" : "Conectar calendario"}
          </button>
        </div>
      </header>
      ...
      {urlEmbed ? (
        <div className="calendario-embed-col">
          {tieneStorageClaude() && (
            <div className="aviso-preview">
              <strong>Nota:</strong> la vista previa de Claude no puede mostrar
              calendarios externos embebidos por una restricción de seguridad
              del entorno, aunque el calendario esté bien conectado. Vas a verlo
              funcionando normalmente una vez publicada la página en Vercel.
              Mientras tanto podés usar "Abrir en Google Calendar" arriba.
            </div>
          )}
        ...
        <div className="panel calendario-vacio">
          ...
          <p className="muted">
            Pegá el ID de tu calendario para verlo acá directamente, con tus
            fechas reales.
          </p>
...
export default function App() {
  const { materias, setMaterias, cargado } = useStore();
  const {
    calendarId,
    setCalendarId,
    cargado: calCargado,
  ...
  return (
    <div className="app-shell">
      <style>{`
        ...
        .historial-panel { margin-top: 18px; }

        .panel-pendientes-grande { margin: 20px 0; border-top: 3px solid var(--ochre); padding: 22px 24px; }
        ...
        .lista-historial { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 2px; }
        .lista-historial li { display: flex; align-items: center; gap: 10px; padding: 9px 6px; border-radius: 8px; cursor: pointer; transition: background 0.15s; }
        .lista-historial li:hover { background: var(--paper-2); }
        .lista-historial li strong { flex: 1; font-size: 13.5px; }
        ...
        .nuevo-recurso { display: grid; grid-template-columns: 110px 1fr 1fr auto; gap: 6px; }
        ...
        .nuevo-examen { display: grid; grid-template-columns: 130px 1fr 70px auto; gap: 6px; }
        ...
        .bloque-agregar { display: flex; gap: 8px; }
      ...
      <Sidebar
        ...
        setView={setView}
        materias={materias}
        onResetear={() => setConfirmarReset(true)}
      />
      ...
      {confirmarReset && (
        ...
        >
          <p style={{ fontSize: 14, lineHeight: 1.5 }}>
            Esto borra todas tus materias actuales y las reemplaza por el set de
            datos de ejemplo (Contador Público, 4 años). No se puede deshacer.
          </p>
          <div className="modal-acciones">
            <button
              className="btn-secundario"
            ...
            <button
              className="btn-primario"
              onClick={confirmarResetDatosEjemplo}
            >
              Sí, restablecer
            </button>
          </div>
