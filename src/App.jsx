...
function CalendarioView({ calendarId, setCalendarId }) {
  ...
  const refrescarEventosMes = async () => {
    ...
    try {
      ...
      const items = await apiListarEventosRango(accessToken, calendarId, inicioMes.toISOString(), finMes.toISOString());
      setEventosMes(items);
    } catch (e) {
  ...
  useEffect(() => {
    if (accessToken && calendarId) refrescarEventosMes();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [accessToken, calendarId, cursor]);
    ...
    <div className="view view-sin-padding-abajo">
        ...
        <div className="calendario-nativo">
          <div className="panel calendario-nativo-panel">
            <div className="calendario-nav">
              ...
              {cargandoEventos ? <span className="muted" style={{ fontSize: 12 }}>Cargando…</span> : <span style={{ width: 62 }} />}
            </div>
      ...
      {eventoModal && (
        <EventoGoogleFormModal
          eventoInicial={formInicialDesdeEvento(eventoModal)}
