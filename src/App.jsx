...
function EventoGoogleFormModal({ eventoInicial, fechaSugerida, onGuardar, onClose, guardando, error }) {
...
function CalendarioView({ calendarId, setCalendarId }) {
  ...
  const [eventoModal, setEventoModal] = useState(null); // null | evento a editar
  ...
  const refrescarEventosMes = async () => {
    ...
    try {
      ...
      const items = await apiListarEventosRango(accessToken, calendarIdNormalizado, inicioMes.toISOString(), finMes.toISOString());
      setEventosMes(items);
    } catch (e) {
      const msg = e.message || "";
  ...
  useEffect(() => {
    if (accessToken && calendarIdNormalizado) refrescarEventosMes();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [accessToken, calendarIdNormalizado, cursor]);
  ...
  const abrirNuevoEvento = () => setEventoModal("nuevo");
  ...
  const eventosPorDia = useMemo(() => {
    ...
    map.forEach((lista) => lista.sort((a, b) => (a.start?.dateTime || "").localeCompare(b.start?.dateTime || "")));
    return map;
  }, [eventosMes]);
        ...
        <div className="calendario-nativo">
          <div className="panel calendario-nativo-panel">
            <div className="calendario-nav">
              <button className="btn-secundario btn-chico" onClick={() => { setCursor(new Date()); setDiaSel(null); }}>Hoy</button>
              <div className="calendario-nav-centro">
                <IconBtn icon={ChevronLeft} title="Mes anterior" onClick={() => cambiarMes(-1)} />
                <strong>{MESES[cursor.getMonth()]} {cursor.getFullYear()}</strong>
                <IconBtn icon={ChevronRight} title="Mes siguiente" onClick={() => cambiarMes(1)} />
              </div>
              <button className="btn-primario btn-chico" onClick={abrirNuevoEvento}>
                ...
                return (
                  ...
                  >
                    ...
                    {evs.slice(0, 3).map((ev) => (
                      <span key={ev.id} className="calendario-evento-chip">{ev.summary || "(sin título)"}</span>
                    ))}
                    {evs.length > 3 && <span className="calendario-evento-mas">+{evs.length - 3} más</span>}
                  </button>
                );
              })}
            </div>
          ...
          {diaSel && (
            <div className="panel calendario-dia-detalle">
              <div className="calendario-dia-detalle-head">
                <h2>{fmtFechaLarga(diaSel)}</h2>
                <IconBtn icon={Plus} title="Agregar evento este día" onClick={abrirNuevoEvento} />
              </div>
  ...
  return (
    <div className="app-shell">
      <style>{`
        ...
        .calendario-evento-chip { width: 100%; font-family: 'IBM Plex Sans', sans-serif; font-size: 10px; background: var(--forest); color: #F6F3E7; border-radius: 4px; padding: 1px 5px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .calendario-evento-mas { font-size: 9.5px; color: var(--ink-soft); }
        .calendario-dia-detalle { flex-shrink: 0; }
        .calendario-dia-detalle-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
        .calendario-dia-detalle-head h2 { margin: 0; }
