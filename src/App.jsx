...
function Inicio({ materias, setView, abrirMateria, onCompletarTarea }) {
  // Materias que todavía no iniciaste (Pendiente), que SÍ tienen correlativas
  // cargadas, y que ya las cumplís todas — es decir, se acaban de habilitar.
  const materiasHabilitadas = useMemo(() => {
    return materias.filter(
      (m) => m.estado === "Pendiente" && (m.correlativas || []).length > 0 && calcularCorrelativasPendientes(m, materias).length === 0
    );
  }, [materias]);
  ...
  return (
    <div className="view">
      {materiasHabilitadas.length > 0 && (
        <section className="banner-habilitadas">
          <GraduationCap size={22} className="banner-habilitadas-icono" />
          <div className="banner-habilitadas-texto">
            <strong>
              {materiasHabilitadas.length === 1
                ? "Ya cumplís las correlativas para cursar:"
                : `Ya cumplís las correlativas para cursar ${materiasHabilitadas.length} materias:`}
            </strong>
            <div className="banner-habilitadas-chips">
              {materiasHabilitadas.map((m) => (
                <button key={m.id} className="banner-habilitadas-chip" onClick={() => abrirMateria(m.id)}>
                  {m.nombre}
                </button>
              ))}
            </div>
...
function FilaMateria({ m, materias, onOpen, onEditar, onEliminar, onCambiarEstado, modo = "normal" }) {
  ...
  const handleClickFila = () => {
    if (esHistorial) setExpandido((v) => !v);
    else onOpen(m.id);
  };

  return (
    <div className={`fila-materia-wrap ${esHistorial ? "fila-materia-wrap-historial" : ""}`}>
    ...
    <Modal title={materia ? "Editar materia" : "Nueva materia"} onClose={onClose} wide>
      <div className="form-grid">
        ...
        <div className="campo campo-full">
          <span>Correlativas (materias que hay que tener aprobadas antes)</span>
...
function MapaCorrelativas({ materias, abrirMateria }) {
  ...
  // Solo se dibuja la línea cuando esa correlativa ya está aprobada (el
  // "camino ya recorrido"). Si todavía está pendiente, no se traza nada:
  // el color gris del nodo bloqueado ya avisa que falta esa materia.
  const lineas = useMemo(() => {
    ...
    materias.forEach((m) => {
      ...
      (m.correlativas || []).forEach((reqId) => {
        ...
        if (!req || req.estado !== "Aprobada") return;
        ...
        const midX = (x1 + x2) / 2;
        out.push({ id: `${req.id}-${m.id}`, d: `M ${x1},${y1} C ${midX},${y1} ${midX},${y2} ${x2},${y2}` });
      });
    });
    return out;
  }, [materias, posiciones]);
  ...
  return (
    <div className="mapa-wrap">
      <div className="mapa-leyenda">
        <span><i className="mapa-leyenda-dot" style={{ background: MAPA_NODO_COLOR.aprobada }} /> Aprobada</span>
        <span><i className="mapa-leyenda-dot" style={{ background: MAPA_NODO_COLOR.habilitada }} /> Habilitada — ya podés cursarla</span>
        <span><i className="mapa-leyenda-dot" style={{ background: MAPA_NODO_COLOR.bloqueada }} /> Bloqueada por correlativas</span>
        <span><i className="mapa-leyenda-linea mapa-leyenda-linea-ok" /> Correlativa cumplida</span>
      </div>
      <div className="mapa-scroll">
        <div className="mapa-lienzo" style={{ width: anchoTotal, height: altoTotal }}>
          ...
          <svg className="mapa-svg" width={anchoTotal} height={altoTotal}>
            {lineas.map((l) => (
              <path key={l.id} d={l.d} fill="none" stroke="#6FB37E" strokeWidth={2} />
            ))}
          </svg>
...
function CalendarioSemana({ dias, eventosPorDia, diaSel, setDiaSel, onEditarEvento, onNuevoEnSlot, materias }) {
  const horas = [];
            ...
            <div key={fechaStr} className="semana-columna" style={{ gridColumn: di + 2, gridRow: `2 / span ${horas.length}` }}>
              {horas.map((h) => (
                <button
                  ...
                  onClick={() => onNuevoEnSlot(fechaStr, h)}
                  aria-label={`Agregar evento a las ${String(h).padStart(2, "0")}:00`}
                />
              ))}
...
function CalendarioView({ calendarId, setCalendarId, materias }) {
  ...
  const abrirNuevoEventoEnSlot = (fechaStr, hora) => {
    setDiaSel(fechaStr);
    setHoraSugerida(`${String(hora).padStart(2, "0")}:00`);
    setEventoModal("nuevo");
  };
        ...
        <div className="calendario-nativo">
          <div className="panel calendario-nativo-panel">
            ...
            {vista === "semana" ? (
              <CalendarioSemana
                ...
                diaSel={diaSel}
                setDiaSel={setDiaSel}
                onEditarEvento={(ev) => setEventoModal(ev)}
                onNuevoEnSlot={abrirNuevoEventoEnSlot}
                materias={materias}
              />
            ) : (
              <>
          ...
          {diaSel && !eventoModal && !eventoAEliminar && (
            <Modal title={fmtFechaLarga(diaSel)} onClose={() => setDiaSel(null)}>
              <button className="btn-primario" style={{ marginBottom: 14 }} onClick={abrirNuevoEvento}>
    ...
    <div className="app-shell">
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .banner-habilitadas, .fila-historial, .subtareas-panel {
        ...
        /* Dashboard */
        .banner-habilitadas { display: flex; align-items: flex-start; gap: 14px; background: color-mix(in srgb, var(--forest) 12%, var(--card)); border: 1.5px solid var(--forest); border-radius: 12px; padding: 16px 20px; margin-bottom: 18px; animation: revealIn 0.18s ease-out; }
        .banner-habilitadas-icono { color: var(--forest); flex-shrink: 0; margin-top: 2px; }
        .banner-habilitadas-texto { display: flex; flex-direction: column; gap: 9px; }
        .banner-habilitadas-texto strong { font-family: 'Fraunces', serif; font-size: 15px; color: var(--forest); }
        .banner-habilitadas-chips { display: flex; flex-wrap: wrap; gap: 8px; }
        .banner-habilitadas-chip { font-family: inherit; font-size: 12.5px; font-weight: 600; color: var(--forest); background: var(--card); border: 1.5px solid var(--forest); border-radius: 20px; padding: 5px 13px; cursor: pointer; transition: background 0.15s; }
        .banner-habilitadas-chip:hover { background: var(--forest); color: #F6F3E7; }
        ...
        .grupo-anio:first-child .fila-materia-wrap:first-child .fila-materia, .lista-materias-panel > .fila-materia-wrap:first-child .fila-materia { border-top: none; }
        ...
        .selector-correlativas { display: flex; flex-direction: column; gap: 2px; max-height: 220px; overflow-y: auto; border: 1px solid var(--line); border-radius: 8px; padding: 4px; }
        ...
        .selector-correlativas-item input { margin: 0; flex-shrink: 0; }
        .selector-correlativas-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
        .selector-correlativas-nombre { flex: 1; font-size: 13px; }
        ...
        .calendario-celda-hoy .calendario-celda-num { color: var(--ochre); font-weight: 700; }
        ...
        .calendario-evento-chip-importante { width: calc(100% + 6px); margin: 0 -3px; padding: 2.5px 8px; font-weight: 700; box-shadow: 0 1px 3px rgba(35,39,31,0.28); }
        .calendario-evento-mas { font-size: 9.5px; color: var(--ink-soft); }
        .calendario-clases-barra { display: flex; width: 100%; height: 15px; border-radius: 4px; overflow: hidden; margin-top: 1px; }
        .calendario-clases-seg { flex: 1; min-width: 0; display: flex; align-items: center; justify-content: center; font-family: 'IBM Plex Sans', sans-serif; font-size: 8px; font-weight: 700; color: #fff; text-shadow: 0 1px 1px rgba(0,0,0,0.35); overflow: hidden; white-space: nowrap; padding: 0 1px; }
        .calendario-clases-seg + .calendario-clases-seg { border-left: 1px solid rgba(255,255,255,0.5); }
        ...
        .tarea-progreso { font-family: 'IBM Plex Mono', monospace; font-size: 10px; font-weight: 700; color: var(--ink-soft); background: var(--paper-2); border-radius: 10px; padding: 1px 7px; flex-shrink: 0; }
        ...
        .subtareas-lista li { display: flex; align-items: center; gap: 8px; padding: 4px 6px; border-radius: 6px; font-size: 12.5px; }
        .subtareas-lista li:hover { background: var(--card); }
        .subtareas-lista li span { flex: 1; min-width: 0; }
        .subtarea-hecha span { text-decoration: line-through; opacity: 0.55; }
        ...
        .mapa-leyenda-dot { display: inline-block; width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
        ...
        .mapa-nodo-bloqueada { opacity: 0.6; filter: grayscale(0.55); }
