              ...
              <div className="fila-menu-pop calendario-menu-pop">
                ...
                {calendarId && (
                  <button
                    onClick={() => {
                      ...
                      window.open(`https://calendar.google.com/calendar/u/0/r?cid=${encodeURIComponent(calendarIdNormalizado)}`, "_blank", "noreferrer");
                    }}
                  >
                    <CalendarIcon size={13} /> Abrir en Google Calendar
                  </button>
                )}
                ...
                {calendarId && (
                  <>
                    <div className="fila-menu-separador" />
                    {!clientId ? (
                      <button onClick={() => { setMenuConfigAbierto(false); setClientModalAbierto(true); }}>
                        <Pencil size={13} /> Usar el calendario nativo, conectado a Google
                      </button>
                    ) : enClaude ? (
                      <p className="fila-menu-nota muted">
                        El calendario conectado solo funciona en tu página publicada, no en esta vista previa.
                      </p>
                    ) : !accessToken ? (
                      <>
                        <button onClick={conectar} disabled={conectando || !scriptListo}>
                          <Pencil size={13} /> {conectando ? "Conectando…" : "Conectar con Google"}
                        </button>
                        <button onClick={() => { setMenuConfigAbierto(false); setClientModalAbierto(true); }}>
                          <Pencil size={13} /> Cambiar credenciales
                        </button>
                      </>
                    ) : (
                    ...
                    {errorAuth && <p className="adjuntar-archivo-error fila-menu-error">{errorAuth}</p>}
                  </>
                )}
              </div>
            </>
  ...
  return (
    <div className="app-shell">
      <style>{`
        ...
        .calendario-menu-pop { min-width: 250px; }
        .calendario-menu-pop button:disabled { opacity: 0.55; cursor: default; }
        .fila-menu-nota { font-size: 12px; line-height: 1.5; padding: 8px 12px; margin: 0; max-width: 230px; }
        .fila-menu-error { padding: 6px 12px 8px; margin: 0; }
