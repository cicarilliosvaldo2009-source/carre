...
function leerArchivoComoDataUrl(file) {
  return new Promise((resolve, reject) => {
    if (file.size > LIMITE_ARCHIVO_BYTES) {
      reject(new Error("El archivo pesa más de 1.5 MB. Subilo a Drive u otro servicio y pegá el link en vez de adjuntarlo."));
      return;
    }
    const reader = new FileReader();
    reader.onerror = () => reject(new Error("No se pudo leer el archivo"));
        ...
        {tab === "examenes" && (
          <div className="tab-panel">
            ...
            <div className="nuevo-examen">
              ...
              />
              <button className="btn-secundario btn-chico" onClick={addExamen}><Plus size={14} /> Agregar</button>
            </div>
          </div>
        )}
        ...
        {tab === "recursos" && (
          <div className="tab-panel">
            ...
            <div className="nuevo-recurso">
              ...
              />
              <button className="btn-secundario btn-chico" onClick={addRecurso}><Plus size={14} /> Agregar</button>
            </div>
            <div className="adjuntar-archivo">
              ...
              <span className="adjuntar-archivo-ayuda">Máx. 1.5 MB — para archivos más grandes, usá el campo URL con un link de Drive.</span>
            </div>
        ...
        {tab === "resumenes" && (
          <div className="tab-panel resumenes-layout">
            <div className="resumenes-lista">
              ...
              <button className="btn-secundario btn-chico" onClick={addResumen}><Plus size={14} /> Nuevo resumen</button>
            </div>
              ...
              {resumen ? (
                <>
                  ...
                  <div className="bloque-agregar">
                    <button className="btn-secundario btn-chico" onClick={() => addBloque(resumen.id, "texto")}>
                      <Type size={14} /> Bloque de texto
                    </button>
                    <button className="btn-secundario btn-chico" onClick={() => addBloque(resumen.id, "imagen")}>
...
function MateriasView({ materias, setMaterias, materiaAbiertaId, setMateriaAbiertaId }) {
  ...
  const filaProps = { materias, onOpen: setMateriaAbiertaId, onEditar: abrirEdicion, onEliminar: eliminarMateria, onCambiarEstado: cambiarEstado };
  ...
  return (
    <div className="app-shell">
      <style>{`
        ...
        .calendario-embed { width: 100%; height: calc(100% + 84px); min-height: 644px; margin-bottom: -84px; border: none; display: block; }
      ...
      {confirmarReset && (
        <Modal title="Restablecer datos de ejemplo" onClose={() => setConfirmarReset(false)}>
          ...
          <div className="modal-acciones">
            ...
            <button className="btn-primario" onClick={confirmarResetDatosEjemplo}>Sí, restablecer</button>
          </div>
        </Modal>
      )}
    </div>
