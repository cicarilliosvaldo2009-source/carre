...
function MateriaDetalle({ materia, materias, onUpdate, onDelete, onClose, onEdit }) {
  ...
  const addExamen = () => {
    ...
    patch({
      examenes: [
        ...examenes,
        { id: uid(), tipo: nuevoExamen.tipo, titulo, nota: nuevoExamen.nota === "" ? null : Number(nuevoExamen.nota), fecha: nuevoExamen.fecha },
      ],
    });
...
function MateriasView({ materias, setMaterias, materiaAbiertaId, setMateriaAbiertaId, onAprobada }) {
...
function FocusView({ materias, sesiones, agregarSesion }) {
  ...
  useEffect(() => {
    if (materias.length > 0 && !materias.some((m) => m.id === materiaId)) {
          ...
          <div className="columna-izquierda">
            <section className="panel focus-panel-timer">
              ...
              <div className="focus-timer-wrap">
                <svg viewBox="0 0 200 200" className="focus-timer-svg">
                  <circle cx="100" cy="100" r={FOCUS_RADIO} fill="none" stroke="var(--line)" strokeWidth="10" />
                  <circle
                    cx="100" cy="100" r={FOCUS_RADIO} fill="none"
                    stroke={fase === "descanso" ? "var(--ochre)" : "var(--forest)"}
                ...
                <div className="focus-timer-texto">
                  <strong>{mm}:{ss}</strong>
                  <span className="muted">
                    {fase === "inactivo" && (materiaActiva ? `Listo para estudiar ${materiaActiva.nombre}` : "Elegí una materia")}
  ...
  return (
    <div className={`app-shell ${tema === "oscuro" ? "app-shell-oscuro" : ""}`}>
      <style>{`
        ...
        .focus-timer-texto { position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; text-align: center; padding: 0 20px; }
        .focus-timer-texto strong { font-family: 'IBM Plex Mono', monospace; font-size: 42px; font-weight: 700; color: var(--ink); line-height: 1; }
        .focus-timer-texto span { font-size: 12.5px; }
        .focus-controles { display: flex; gap: 10px; }
        .focus-duraciones { display: flex; gap: 14px; width: 100%; }
