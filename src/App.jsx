...
// de color al editar un evento ahí). Si el evento tiene colorId, usamos ese
// color exacto; si no, elegimos uno estable a partir del título para que
// cada materia/evento se vea siempre del mismo color.
...
function useStore() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (datos) {
        setMaterias((datos.materias || []).map((m) => ({ ...m, notas: normalizarNotasEntradas(m.notas) })));
      } else {
        const seed = seedData();
...
function CalendarioSemana({ dias, eventosPorDia, diaSel, setDiaSel, onEditarEvento, onNuevoEnSlot }) {
  const horas = [];
              ...
              {evs.map((ev) => {
                ...
                return (
                  <button
                    ...
                    style={{ top, height, background: colorParaEvento(ev) }}
                    ...
                    return (
                      ...
                      >
                        ...
                        {evs.slice(0, 3).map((ev) => (
                          <span key={ev.id} className="calendario-evento-chip" style={{ background: colorParaEvento(ev) }}>{ev.summary || "(sin título)"}</span>
                        ))}
              ...
              {eventosDelDiaSel.length === 0 ? (
                ...
                <ul className="lista-eventos-google">
                  {eventosDelDiaSel.map((ev) => (
                    <li key={ev.id}>
                      <span className="lista-eventos-google-dot" style={{ background: colorParaEvento(ev) }} />
  ...
  return (
    <div className="app-shell">
      <style>{`
        ...
        .fila-historial { padding: 4px 18px 18px 39px; border-top: 1px dashed var(--line); background: var(--paper-2); }
